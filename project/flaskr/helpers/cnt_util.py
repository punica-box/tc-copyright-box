...
def check_copyright_to_contract(ont_id: str, copyright: str) -> str:
    check_copyright_func = app.config['ABI_INFO'].get_function('check_copyright')
    ont_id_acct_bytes = ont_id
    check_copyright_func.set_params_value((ont_id_acct_bytes, copyright))
    gas_limit = app.config['GAS_LIMIT']
    gas_price = app.config['GAS_PRICE']
    contract_address_bytearray = app.config['CONTRACT_ADDRESS_BYTEARRAY']
    tx_hash = app.config['ONTOLOGY'].neo_vm().send_transaction(contract_address_bytearray,
                                                               gas_limit, gas_price,
                                                               check_copyright_func, False)
    return tx_hash
